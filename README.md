...
`npm run ig-card -- --tonight` renders a Tonight cover and one card per pick into `web/assets/ig/`, so a push makes them public at standupcomedynyc.com/assets/ig/<id>.jpg, which the Instagram API needs. Cards are text-only in the site's style. Artwork is included only when its record in POSTER-SOURCES.json has `reuse: "granted"`. Paid listings are labeled on the card. Sold-out picks are left out of the Tonight set (they stay on the site with the tag) unless the pick carries a `walkup_note`. Rendering needs a Chromium headless shell: set `IG_BROWSER` or let the script find one under `PLAYWRIGHT_BROWSERS_PATH` or `~/.cache/ms-playwright`. Rotation deletes cards for expired picks and past covers.
