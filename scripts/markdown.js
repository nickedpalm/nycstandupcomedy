...
const big = (r) => venues.get(r.venue)?.kind === 'theater';
...
const soon = picks.filter((p) => p.date <= horizon), later = picks.filter((p) => p.date > horizon);
...
const pickLine = (p) => `- **${p.time_label}** — [${p.title}](${p.ticket_url || p.source_url}) at ${p.venue}, ${hood(p)}. ${p.price_label}.${big(p) ? ' Big stage.' : ''}${p.demand === 'sold_out' ? ' Sold out.' : p.demand === 'going_fast' ? ' Going fast.' : ''}${p.sponsored ? ' Paid listing.' : ''} ${p.description}`;
...
const files = {
  'index.md': head('Stand-Up Comedy NYC', `A curated, dated shortlist of NYC stand-up shows worth going out for, plus the bars and rooms with weekly shows, open mics by night, and the Manhattan clubs. ${picks.length} upcoming picks.`) +
    `Sections: [This week](${SITE}/this-week.md) · [Weekly and monthly rooms](${SITE}/rooms.md) · [Open mics](${SITE}/open-mics.md) · [Clubs](${SITE}/clubs.md) · [Neighborhoods](${SITE}/neighborhoods.md)\n\nData as JSON: [picks](${SITE}/data/picks.json), [recurring](${SITE}/data/recurring.json), [open mics](${SITE}/data/open-mics.json), [clubs](${SITE}/data/clubs.json), [venues](${SITE}/data/venues.json).\n` + byDay(soon) + (later.length ? `\n# Save the date\n\nFurther out than two weeks.\n` + byDay(later) : ''),
  'this-week.md': head('This week', 'Every pick in the next two weeks, by day.') + byDay(soon),
  'save-the-date.md': head('Save the date', 'Verified shows more than two weeks out. Tickets are usually on sale now.') + (later.some(big) ? '\n## Big stages\n\n' + later.filter(big).map((p) => `- **${longDate(p.date)} · ${p.time_label}** — [${p.title}](${p.ticket_url || p.source_url}) at ${p.venue}. ${p.price_label}.`).join('\n') + '\n' : '') + (later.filter((p) => !big(p)).length ? byDay(later.filter((p) => !big(p))) : '\nNothing listed yet beyond two weeks.\n'),
