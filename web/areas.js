...
Promise.all(['/data/venues.json','/data/picks.json','/data/recurring.json','/data/open-mics.json','/data/clubs.json'].map(load)).then(([venues,picks,rooms,mics,clubs])=>{
 ...
 function block(area,full){
  ...
  if(area.picks.length){section.append(element('h4','This week'));const ul=element('ul',undefined,'area-list');area.picks.forEach(p=>ul.append(row(dateLabel(p.date)+' · '+p.time_label,link(p.title,p.ticket_url||p.source_url),[p.venue,(venues.find(v=>v.name===p.venue)||{}).address,p.price_label].filter(Boolean).join(' · '))));section.append(ul);}
  if(area.rooms.length){section.append(element('h4','Every week'));const ul=element('ul',undefined,'area-list');area.rooms.sort((x,y)=>x.weekday-y.weekday).forEach(r=>ul.append(row(WEEKDAYS[r.weekday]+' · '+r.time_label,link(r.title,r.source_url),[r.venue,r.address,r.price_label].filter(Boolean).join(' · '))));section.append(ul);}
  if(area.mics.length){section.append(element('h4','Open mics'));const ul=element('ul',undefined,'area-list');area.mics.sort((x,y)=>x.weekday-y.weekday).forEach(m=>ul.append(row(WEEKDAYS[m.weekday]+' · '+(m.time_label||m.cadence),link(m.title,m.source_url),[m.venue,m.address,m.cost_label].filter(Boolean).join(' · '))));section.append(ul);}
